...
| `config: kek: open deploy/dev-kek.b64: no such file or directory` in a `*-token` job | The job runs without the auth KEK. `docker-compose.yaml.example` does not mount it on those jobs; the overlay does - check `docker-compose.override.yaml` is the production overlay. |
